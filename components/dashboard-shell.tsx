...
export function DashboardShell({
  ...
  return (
    <div className="min-h-screen text-[var(--retro-text)]">
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-5 px-4 py-4 lg:grid lg:grid-cols-[270px_minmax(0,1fr)] lg:items-start lg:px-6 lg:py-6">
        <aside className="retro-panel hidden flex-col rounded-[26px] p-5 lg:sticky lg:top-6 lg:flex lg:h-[calc(100dvh-3rem)] lg:max-h-[calc(100dvh-3rem)] lg:self-start lg:overflow-y-auto">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-5">
          <div className="flex items-center justify-between rounded-[22px] border-2 border-[var(--retro-border)] bg-[var(--retro-panel)] px-4 py-3 shadow-[6px_6px_0_var(--retro-shadow)] lg:hidden">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[var(--retro-accent)]">
                FinTrack
              </p>
              <h1 className="truncate text-lg font-bold text-[var(--retro-text)]">
                {activeLabel}
              </h1>
            </div>
          ...
          <main className="retro-panel flex min-h-[calc(100vh-2rem)] flex-col rounded-[26px] p-4 lg:min-h-[calc(100dvh-3rem)] lg:p-8">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-[100] w-[86vw] max-w-[320px] transform border-r-2 border-[var(--retro-border)] bg-[var(--retro-panel)] p-5 shadow-[12px_0_0_var(--retro-shadow)] transition-transform duration-300 lg:hidden",
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={cn(
                  "flex items-center gap-3 rounded-[18px] border-2 px-4 py-4 text-sm font-semibold uppercase tracking-[0.12em] transition",
                  active
                    ? "border-[var(--retro-border)] bg-[var(--retro-accent)] !text-[var(--retro-ink)]"
                    : "border-[var(--retro-border)] bg-[var(--retro-surface)] text-[var(--retro-text)]",
                )}
